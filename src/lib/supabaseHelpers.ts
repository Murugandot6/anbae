 ...
 * @param profileIds An array of profile IDs to fetch.
 * @returns A Map where keys are profile IDs and values are Profile objects.
 */
export const fetchProfilesByIds = async (profileIds: string[]): Promise<Map<string, Profile>> => {
  ...
  try {
    ...
    if (error) {
      console.error('Supabase Error fetching multiple profiles:', error.message, error);
      return profilesMap;
    }
    ...
    console.error('Unexpected error fetching multiple profiles:', error.message, error);
    return profilesMap;
  }
};